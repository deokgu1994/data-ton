...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, num_classes, pretrainde_pth, **kwargs):
        global ALIGN_CORNERS

        super(HighResolutionNet, self).__init__()
        ALIGN_CORNERS = True # config
        self.num_classes = num_classes
        self.pretrained_path = pretrainde_pth
        ...
        self.stage1_cfg = {'NUM_MODULES':1, "NUM_BRANCHES": 1, "BLOCK":"BOTTLENECK", "NUM_BLOCKS":[4],"NUM_CHANNELS":[64],"FUSE_METHOD": "SUM"}
        ...
        self.ocr_gather_head = SpatialGather_Module(self.num_classes) # cfg
...
class OcrHrnetv2(nn.Module):
    def __init__(self, num_classes,pretrainde_pth):
        super().__init__()        
        self.backbone = HighResolutionNet(num_classes, pretrainde_pth)
        self.pretrainde_pth = pretrainde_pth
        self.init_weights()
    #@autocast()
    def forward(self, x):
        x = self.backbone(x)
    ...
    def init_weights(self,):
        self.backbone.init_weights(self.pretrainde_pth)
        
if __name__ == "__main__":
    model = OcrHrnetv2(num_classes=11, pretrainde_pth = "/opt/ml/segmentation/Pytroch_Template/_Test_ipynb/model/seg/hrnetv2_w48_imagenet_pretrained.pth")
