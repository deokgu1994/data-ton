...
class BaseTrainer:
    ...
    """
    def __init__(self, model, criterion, data_set,device, metric_ftns = None, optimizer =None, config = None):
        self.config = config
    ...
    def train(self):
        ...
        mskf = StratifiedKFold(n_splits=self.config["fold_split"], shuffle=True, random_state=2019)
        for kfold, (train_index, validate_index) in enumerate(mskf.split(self.data_set, self.data_set.labels)):
            print(len(train_index), len(validate_index))
            
            train_dataset = torch.utils.data.dataset.Subset(self.data_set, train_index)
            val_data_set = copy.deepcopy(self.data_set)
            val_data_set.mode = "val"
            valid_dataset = torch.utils.data.dataset.Subset(val_data_set, validate_index)
